...
exports.CharacterPhaseRegistry = void 0;
exports.createPropagationPhase = createPropagationPhase;
exports.createGoalPhase = createGoalPhase;
exports.createInfluencePhase = createInfluencePhase;
...
class CharacterPhaseRegistry {
    configs = new Map();
    goalManagers = new Map();
    influenceTracker = new influence_1.InfluenceTracker();
    alreadyToldRecord = new propagation_1.AlreadyToldRecord();
    /**
    ...
    register(entityId, config) {
        ...
        if (config.goalDefs && config.goalDefs.length > 0) {
            const manager = new goals_1.GoalManager();
    ...
    restoreState(saved) {
        ...
        if (saved.influenceEffects) {
            const restored = influence_1.InfluenceTracker.fromJSON(saved.influenceEffects);
            // Copy effects into the existing tracker
            for (const effect of restored.toJSON()) {
                this.influenceTracker.track(effect.influenceName, effect.influencerId, effect.targetId, effect.effect, {
        ...
        if (saved.alreadyTold) {
            propagation_1.AlreadyToldRecord.fromJSON(saved.alreadyTold);
...
exports.CharacterPhaseRegistry = CharacterPhaseRegistry;
// ---------------------------------------------------------------------------
// Event helper
// ---------------------------------------------------------------------------
 ...
 */
function handleRoomPropagation(roomId, roomNpcList, registry, world, turn, playerLocation, events) {
    for (const speaker of roomNpcList) {
        const config = registry.getConfig(speaker.id);
        const trait = speaker.get(world_model_1.TraitType.CHARACTER_MODEL);
        if (!trait || !config.propagationProfile)
            continue;
        const listeners = roomNpcList
            .filter(n => n.id !== speaker.id)
            .map(n => ({
            id: n.id,
            trait: n.get(world_model_1.TraitType.CHARACTER_MODEL),
        ...
        const transfers = (0, propagation_1.evaluatePropagation)(propContext);
 ...
 * @param events - Accumulator for witnessed events
 */
function recordTransfer(transfer, speaker, roomId, registry, world, turn, playerLocation, events) {
    const listenerEntity = world.getEntity(transfer.listenerId);
    if (!listenerEntity)
        return;
    const listenerTrait = listenerEntity.get(world_model_1.TraitType.CHARACTER_MODEL);
    if (!listenerTrait)
        return;
    ...
    const result = (0, propagation_1.transferFact)(transfer, listenerTrait, registry.alreadyToldRecord, turn, receivesAs);
    if (roomId === playerLocation && !result.alreadyKnew) {
        const visibility = (0, propagation_1.getVisibilityResult)(transfer, 'present');
 ...
 * @returns Tick phase handler function
 */
function createGoalPhase(registry) {
    return (npcs, ctx) => {
        const events = [];
 ...
 * @param events - Accumulator for witnessed events
 */
function executeNpcGoals(npc, registry, world, playerLocation, events) {
    const manager = registry.getGoalManager(npc.id);
    if (!manager)
        return;
    const trait = npc.get(world_model_1.TraitType.CHARACTER_MODEL);
    if (!trait)
        return;
    ...
    const movement = config?.movementProfile ?? { knows: 'all', access: 'all' };
    const stepContext = {
        npcId: npc.id,
    ...
    const stepResult = (0, goals_1.evaluateGoalStep)(activeGoal, stepContext);
    if ((stepResult.status === 'completed' || stepResult.status === 'in-progress') &&
 ...
 * @returns Tick phase handler function
 */
function createInfluencePhase(registry) {
    return (npcs, ctx) => {
        const events = [];
        ...
        for (const npc of npcs) {
            ...
            roomEntities.set(loc, [
                ...(roomEntities.get(loc) ?? []),
                {
                    ...
                    evaluatePredicate: (pred) => trait ? trait.evaluate(pred) : false,
                },
            ]);
        }
        ...
        for (const [roomId, entities] of roomEntities) {
            const results = (0, influence_1.evaluatePassiveInfluences)(entities);
        ...
        const expired = registry.influenceTracker.expireTurn(turn, (targetId, pred) => {
            ...
            if (!entity)
                return false;
            const trait = entity.get(world_model_1.TraitType.CHARACTER_MODEL);
 ...
 * @param events - Accumulator for witnessed events
 */
function handleInfluenceResults(results, roomId, registry, world, turn, playerLocation, events) {
    for (const result of results) {
        if (result.status === 'applied') {
            const influencerConfig = registry.getConfig(result.influencerId);
            const influenceDef = influencerConfig?.influenceDefs?.find(d => d.name === result.influenceName);
            registry.influenceTracker.track(result.influenceName, result.influencerId, result.targetId, result.effect, {
...
function buildRoomGraph(world) {
    const graph = new goals_1.SimpleRoomGraph();
    const allEntities = world.getAllEntities();
    for (const entity of allEntities) {
        const roomTrait = entity.get(world_model_1.RoomTrait);
        if (!roomTrait?.exits)
            continue;
...
//# sourceMappingURL=tick-phases.js.map