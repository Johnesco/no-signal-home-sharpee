...
const event_processor_1 = require("../event-processor/index.js");
...
const plugins_1 = require("../plugins/index.js");
...
class GameEngine {
    ...
    setStory(story) {
        ...
        if (this.parser && 'setDirectionVocabulary' in this.parser) {
            this.world.directions().onVocabularyChange((vocab) => {
                this.parser.setDirectionVocabulary(vocab);
            });
        }
    ...
    start() {
        ...
        const storyInfoEntities = this.world.findByTrait(world_model_1.TraitType.STORY_INFO);
        const storyInfoTrait = storyInfoEntities[0]?.get(world_model_1.StoryInfoTrait);
    ...
    configureLanguageProviderNarrative(player) {
        ...
        if (this.narrativeSettings.perspective === '3rd') {
            ...
            if (this.narrativeSettings.playerPronouns) {
                ...
                const actorTrait = player.get(world_model_1.ActorTrait);
     ...
     */
    executeInputMode(input, handler, turn) {
     ...
     */
    appendPromptBlock(blocks) {
        if (!this.languageProvider || !this.world)
            return;
        const prompt = this.world.getPrompt();
        const resolved = this.languageProvider.getMessage(prompt.messageId, prompt.params);
        // Only append if the message resolved (not echoed back as the ID)
        if (resolved && resolved !== prompt.messageId) {
            blocks.push({ key: text_blocks_1.BLOCK_KEYS.PROMPT, content: [resolved] });
        }
    }
    /**
