        ...
        for (const constraint of slotConstraints.constraints) {
            if (typeof constraint === 'function') {
                ...
                if (constraint.length === 1) {
                    ...
                    const matchingEntities = grammar_scope_resolver_1.GrammarScopeResolver.findEntitiesByName(slotText, scopeConstraint, context);
                    if (process.env.PARSER_DEBUG === 'true') {
                        ...
                        const allInScope = grammar_scope_resolver_1.GrammarScopeResolver.getEntitiesInScope({ ...scopeConstraint, filters: [] }, // Without filters
                        context);
                        console.log(`  Entities in ${scopeConstraint.base} scope: ${allInScope.length}`);
                        for (const e of allInScope.slice(0, 5)) {
                            const eAny = e;
                            const getFn = eAny.get;
