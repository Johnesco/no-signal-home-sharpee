        ...
        if (item.type === 'command') {
            ...
            if (!result.passed && !result.expectedFailure && !result.skipped) {
                ...
                if (retryBlock) {
                    ...
                    if (retryBlock.retryCount <= retryBlock.maxRetries) {
                        ...
                        // Restore world state
                        if (retryBlock.savedState && engine.world && engine.world.loadJSON) {
            ...
            if (directive.type === 'ensures') {
                ...
                if (retryBlock && engine.world && directive.condition) {
                    const condResult = (0, condition_evaluator_1.evaluateCondition)(directive.condition, engine.world, playerId, lastOutput);
                    if (!condResult.met) {
                        retryBlock.retryCount = (retryBlock.retryCount ?? 0) + 1;
                        if (retryBlock.retryCount <= retryBlock.maxRetries) {
                            if (options.verbose) {
            ...
            const directiveResult = await handleDirective(directive, blockStack, i, items, engine, playerId, options, lastOutput);
 ...
 */
function findRetryBlock(blockStack) {
 ...
 */
function findDoBlock(blockStack) {
    for (let i = blockStack.length - 1; i >= 0; i--) {
 ...
 */
function findWhileBlock(blockStack) {
    for (let i = blockStack.length - 1; i >= 0; i--) {
 ...
 */
async function handleDirective(directive, blockStack, currentIndex, items, engine, playerId, options, lastOutput) {
    ...
    switch (directive.type) {
        case 'goal': {
            ...
            if (world) {
                for (const req of requires) {
                    const result = (0, condition_evaluator_1.evaluateCondition)(req, world, playerId, lastOutput);
                    if (verbose) {
        ...
        case 'end_goal': {
            ...
            if (world && block.ensures) {
                for (const ens of block.ensures) {
                    const result = (0, condition_evaluator_1.evaluateCondition)(ens, world, playerId, lastOutput);
                    if (verbose) {
        ...
        case 'if': {
            ...
            if (world && directive.condition) {
                const result = (0, condition_evaluator_1.evaluateCondition)(directive.condition, world, playerId, lastOutput);
        ...
        case 'end_while': {
            ...
            if (world && block.condition) {
                const result = (0, condition_evaluator_1.evaluateCondition)(block.condition, world, playerId, lastOutput);
                conditionMet = result.met;
                if (verbose) {
        ...
        case 'until': {
            ...
            const matched = block.iterationOutputs?.some(output => {
 ...
 */
function captureEntityTraits(data, world) {
    const candidates = [];
...
async function runCommand(command, engine, options) {
    ...
    if (!normalizedActual && !error) {
        return {
            ...
            skipped: false,
            assertionResults: [{
                    assertion: { type: 'ok-contains', value: '(any output)' },
                    passed: false,
