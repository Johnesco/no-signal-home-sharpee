...
async function main() {
    ...
    if (options.play) {
        ...
        let game;
        try {
            game = await (0, story_loader_1.loadStory)(options.storyPath);
        }
        catch (error) {
    ...
    if (options.all || transcriptPaths.length === 0) {
        ...
        if (fs.existsSync(testsDir)) {
            transcriptPaths = (0, story_loader_1.findTranscripts)(testsDir);
        }
        else {
            // Check for transcripts directory
            const transcriptsDir = path.join(options.storyPath, 'tests', 'transcripts');
            if (fs.existsSync(transcriptsDir)) {
                transcriptPaths = (0, story_loader_1.findTranscripts)(transcriptsDir);
            }
        }
    }
    // Deduplicate paths
    ...
    for (const transcriptPath of transcriptPaths) {
        ...
        (0, reporter_1.reportTranscript)(result, { verbose: options.verbose, emitTraits: options.emitTraits });
    ...
    if (options.outputDir) {
        const timestamp = (0, reporter_1.generateTimestamp)();
        const jsonPath = (0, reporter_1.writeResultsToJson)(runResult, options.outputDir, timestamp);
        const reportPath = (0, reporter_1.writeReportToFile)(runResult, options.outputDir, timestamp);
        console.log();
