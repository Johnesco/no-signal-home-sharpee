...
exports.formatEntityTraitLines = formatEntityTraitLines;
exports.formatTraitProse = formatTraitProse;
/**
 ...
 */
function formatEntityTraitLines(snapshots) {
    const lines = [];
    for (const snapshot of snapshots) {
        const identity = snapshot.traits['identity'];
 ...
 */
function buildHeader(entityId, identity) {
    if (!identity)
        return entityId;
    const parts = [];
    ...
    const tags = [];
    if (identity.weight != null)
        tags.push(`wt ${identity.weight}`);
    if (identity.points != null)
        tags.push(`take-pts ${identity.points}`);
    if (identity.concealed)
        tags.push('concealed');
    if (tags.length) {
 ...
 */
function formatTraitProse(traitType, props) {
    switch (traitType) {
        case 'room':
...
function formatRoom(p) {
    ...
    if (p.exits && typeof p.exits === 'object') {
        ...
        if (exitList.length)
            parts.push(`exits: ${exitList.join(', ')}`);
        else
            parts.push('no exits');
    }
    // Flags
    ...
    if (p.isOutdoors)
        flags.push('outdoors');
    if (p.isUnderground)
        flags.push('underground');
    if (p.visited === false)
        flags.push('unvisited');
    if (flags.length)
        parts.push(flags.join(', '));
    return `room: ${parts.join('; ')}`;
}
function formatActor(p) {
    if (p.isPlayer)
        return 'player';
    return 'NPC actor';
}
function formatContainer(p) {
    const cap = p.capacity;
    if (!cap)
        return 'container';
    const parts = [];
    if (cap.maxItems != null)
        parts.push(`${cap.maxItems} items`);
    if (cap.maxWeight != null)
        parts.push(`${cap.maxWeight} wt`);
    return `container: ${parts.join(' / ') || 'unlimited'}`;
}
function formatOpenable(p) {
    return p.isOpen ? 'open' : 'closed';
}
function formatLockable(p) {
    const parts = [p.isLocked ? 'locked' : 'unlocked'];
    if (p.keyId)
        parts.push(`key: ${p.keyId}`);
    return parts.join(', ');
}
function formatLightSource(p) {
    const parts = [p.isLit ? 'lit' : 'unlit'];
...
function formatSwitchable(p) {
    return p.isOn ? 'on' : 'off';
}
function formatWeapon(p) {
    const parts = [];
    if (p.weaponType)
        parts.push(p.weaponType);
    if (p.damage != null)
        parts.push(`dmg ${p.damage}`);
    if (p.skillBonus)
        parts.push(`skill +${p.skillBonus}`);
    if (p.twoHanded)
        parts.push('two-handed');
    if (p.isBlessed)
        parts.push('blessed');
    if (p.glowsNearDanger && p.isGlowing)
        parts.push('GLOWING');
    return `weapon: ${parts.join(', ')}`;
}
function formatCombatant(p) {
    const parts = [];
    if (p.health != null && p.maxHealth != null)
        parts.push(`HP ${p.health}/${p.maxHealth}`);
    if (p.skill != null)
        parts.push(`skill ${p.skill}`);
    if (p.baseDamage != null)
        parts.push(`dmg ${p.baseDamage}`);
    if (p.hostile)
        parts.push('HOSTILE');
    if (!p.isAlive)
        parts.push('DEAD');
...
function formatNpc(p) {
    ...
    if (custom?.state)
        parts.push(custom.state.toLowerCase());
...
function formatPushable(p) {
    const parts = [];
    if (p.pushType)
        parts.push(p.pushType);
    if (p.pushCount > 0)
        parts.push(`pushed ${p.pushCount}x`);
    if (p.state && p.state !== 'default')
        parts.push(p.state);
    return `pushable: ${parts.join(', ') || 'moveable'}`;
}
function formatTreasure(p) {
    const parts = [];
    if (p.trophyCaseValue != null)
        parts.push(`trophy case ${p.trophyCaseValue} pts`);
    if (p.trophyCaseDescription)
        parts.push(p.trophyCaseDescription);
    return `treasure: ${parts.join(', ')}`;
}
function formatUnknown(traitType, props) {
    ...
    if (compact.length <= 80)
        return `${traitType}: ${compact}`;
    // Truncate if very long
    return `${traitType}: ${compact.substring(0, 77)}...`;
}
//# sourceMappingURL=trait-formatter.js.map