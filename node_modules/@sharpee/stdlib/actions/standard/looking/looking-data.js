...
exports.listContentsDataConfig = exports.roomDescriptionDataConfig = exports.lookingEventDataConfig = exports.buildListContentsData = exports.buildRoomDescriptionData = exports.buildLookingEventData = void 0;
exports.determineLookingMessage = determineLookingMessage;
 ...
 */
function checkIfDark(context) {
    const room = context.world.getContainingRoom(context.player.id);
    if (!room)
        return false;
    return world_model_1.VisibilityBehavior.isDark(room, context.world);
}
/**
 * Build looking action data for the looked event
 */
const buildLookingEventData = (context, preState, postState) => {
    const player = context.player;
    // Use visibility logic to determine what location to describe
    const { location, immediateContainer } = world_model_1.VisibilityBehavior.getDescribableLocation(player, context.world);
    // Get visible items (excluding the room itself and the player)
    const visible = context.getVisible().filter(e => e.id !== location.id && e.id !== player.id);
    // Check if dark
    const isDark = checkIfDark(context);
    // Create atomic looked event with complete room snapshot
    const roomSnapshot = (0, snapshot_utils_1.captureRoomSnapshot)(location, context.world, false);
    const visibleSnapshots = (0, snapshot_utils_1.captureEntitySnapshots)(visible, context.world);
    // Get identity trait for ID fields (ADR-107 dual-mode)
    const identity = location.getTrait(world_model_1.IdentityTrait);
    return {
        actorId: player.id,
...
exports.buildLookingEventData = buildLookingEventData;
/**
 * Build room description event data
 */
const buildRoomDescriptionData = (context, preState, postState) => {
    // Use visibility logic to determine what location to describe
    const { location, immediateContainer } = world_model_1.VisibilityBehavior.getDescribableLocation(context.player, context.world);
    // Get visible items (excluding the room itself and the player)
    const visible = context.getVisible().filter(e => e.id !== location.id && e.id !== context.player.id);
    // Create snapshots
    const roomSnapshot = (0, snapshot_utils_1.captureRoomSnapshot)(location, context.world, false);
    const visibleSnapshots = (0, snapshot_utils_1.captureEntitySnapshots)(visible, context.world);
...
exports.buildRoomDescriptionData = buildRoomDescriptionData;
/**
 * Build list contents event data
 */
const buildListContentsData = (context, preState, postState) => {
    // Use visibility logic to determine what location to describe
    const { location } = world_model_1.VisibilityBehavior.getDescribableLocation(context.player, context.world);
    // Get visible items (excluding the room itself and the player)
    const visible = context.getVisible().filter(e => e.id !== location.id && e.id !== context.player.id);
    if (visible.length === 0) {
    ...
    const npcs = directInRoom.filter(e => e.hasTrait(world_model_1.TraitType.ACTOR));
    const containers = directInRoom.filter(e => e.hasTrait(world_model_1.TraitType.CONTAINER) && !e.hasTrait(world_model_1.TraitType.ACTOR));
    const supporters = directInRoom.filter(e => e.hasTrait(world_model_1.TraitType.SUPPORTER) && !e.hasTrait(world_model_1.TraitType.CONTAINER));
    const otherItems = directInRoom.filter(e => !e.hasTrait(world_model_1.TraitType.ACTOR) &&
        !e.hasTrait(world_model_1.TraitType.CONTAINER) &&
        !e.hasTrait(world_model_1.TraitType.SUPPORTER));
    // Build container/supporter contents info for separate rendering
    const openContainerContents = [];
    // Check each container for visible contents
    for (const container of containers) {
        // Skip if container is closed
        if (container.hasTrait(world_model_1.TraitType.OPENABLE)) {
            const { OpenableBehavior } = require("../../../../world-model/index.js");
            if (!OpenableBehavior.isOpen(container))
                continue;
        }
...
exports.buildListContentsData = buildListContentsData;
/**
 * Determine the appropriate message ID and parameters for looking
 */
function determineLookingMessage(context, isDark) {
    // Use visibility logic to determine what location to describe
    const { location, immediateContainer } = world_model_1.VisibilityBehavior.getDescribableLocation(context.player, context.world);
    const params = {};
    if (isDark) {
    ...
    if (!immediateContainer && location.hasTrait(world_model_1.TraitType.CONTAINER)) {
        return {
    ...
    else if (!immediateContainer && location.hasTrait(world_model_1.TraitType.SUPPORTER)) {
        return {
 ...
 */
exports.lookingEventDataConfig = {
    builder: exports.buildLookingEventData,
    protectedFields: ['actorId', 'locationId', 'timestamp']
};
exports.roomDescriptionDataConfig = {
    builder: exports.buildRoomDescriptionData,
    protectedFields: ['roomId', 'timestamp']
};
exports.listContentsDataConfig = {
    builder: exports.buildListContentsData,
    protectedFields: ['locationName', 'timestamp']
};
//# sourceMappingURL=looking-data.js.map