...
export function createRooms(world: WorldModel): RoomIds {
  ...
  airlock.add(new IdentityTrait({
    name: 'Airlock',
    description: 'A cylindrical airlock chamber connecting the tug to The Stillwater. Biohazard warning decals peel from the inner hull. Emergency lighting casts everything in pale amber. The tug is north. The ship corridor stretches south.',
    properName: true,
  }));
  ...
  forwardCorridor.add(new IdentityTrait({
    ...
    description: 'A long corridor running through the bow of The Stillwater. Overhead pipes drip condensation. The air smells of machine oil and something faintly chemical. The airlock is north. The corridor continues south toward the cargo bay. A maintenance hatch opens east.',
    properName: true,
  }));
  ...
  maintenanceShaft.add(new IdentityTrait({
    ...
    description: 'A narrow cable conduit running between decks. Bundles of fiber-optic cable snake along the walls. The air is stale and close. Your flashlight picks out junction boxes bolted to the bulkhead. The corridor is west. The shaft continues south to engineering.',
    properName: true,
  }));
  ...
  cargoBay.add(new IdentityTrait({
    name: 'Cargo Bay',
    description: 'A cavernous cargo bay with high ceilings lost in shadow. Shipping containers stand in rows, some pried open. Someone has made a camp here — improvised bedding, a makeshift barricade of crates. The corridor is north. A sealed bulkhead leads south to the cargo hold. Storage is east.',
    properName: true,
  }));
  ...
  cargoHold.add(new IdentityTrait({
    name: 'Cargo Hold',
    description: 'The deep cargo hold. Warning labels cover every surface. Rows of sealed transport containers fill the space, stamped with Meridian Solutions logos and biohazard symbols. The air tastes wrong — metallic, with something organic underneath. The cargo bay is north.',
    properName: true,
  }));
  ...
  storageAnnex.add(new IdentityTrait({
    name: 'Storage Annex',
    description: 'A supply room lined with industrial shelving. Spare parts, sealed ration containers, and maintenance equipment fill the racks. A yellow hazmat suit hangs from a hook near the door. The cargo bay is west.',
    properName: true,
  }));
  ...
  lowerMidCorridor.add(new IdentityTrait({
    ...
    description: 'A junction point where the forward and aft sections meet. Direction signs point fore and aft, their corporate fonts cheerful against the grimy walls. A ladder leads up through a ceiling hatch. The corridor runs north and south. An elevator alcove opens east.',
    properName: true,
  }));
  ...
  cryoBay.add(new IdentityTrait({
    name: 'Cryo Bay',
    description: 'Rows upon rows of cryo pods stretch into the gloom, stacked three high. Status lights blink — green, amber, red, dark. Hundreds of people frozen in here. Some pods have leaked fluid that has pooled on the deck. A chemical haze hangs in the air. Cryo control is south.',
    properName: true,
  }));
  ...
  cryoControl.add(new IdentityTrait({
    name: 'Cryo Control',
    description: 'A control room with a wide observation window overlooking the cryo bay. Banks of monitors display pod status — too many amber and red indicators. A control console offers selective thaw and emergency purge options. The cryo bay is north.',
    properName: true,
  }));
  ...
  aftCorridor.add(new IdentityTrait({
    name: 'Aft Corridor',
    description: 'The corridor narrows toward the stern. The deck plates are warm underfoot. A low thrum vibrates through the walls — the reactor, waking up. Engineering is south. The mid corridor is north.',
    properName: true,
  }));
  ...
  engineering.add(new IdentityTrait({
    name: 'Engineering',
    description: 'The engineering section. Heavy machinery fills the space — power conduits, coolant pipes, switching panels. A tool rack hangs on the far wall. A comms relay unit is bolted beside the main console. The reactor room is east. The aft corridor is north. The maintenance shaft opens to the northwest.',
    properName: true,
  }));
  ...
  reactorRoom.add(new IdentityTrait({
    name: 'Reactor Room',
    description: 'The reactor chamber. Heat radiates from the central core. Warning indicators flash across the containment display. Radiation shielding panels line the walls but some have buckled. Without protection, you should not stay long. Engineering is west.',
    properName: true,
  }));
  ...
  centralJunction.add(new IdentityTrait({
    ...
    description: 'A wide junction where the mid deck splits into two wings. Corporate signage directs crew west to labs and medical, east to habitation. A Meridian Solutions poster reads: "People First. Profit Always." Ladders lead up and down.',
    properName: true,
  }));
  ...
  labCorridor.add(new IdentityTrait({
    name: 'Lab Corridor',
    description: 'A corridor that was once sterile white, now streaked with grime. An old chemical spill has dried to a dark stain on the deck. A safety station with eye wash and gloves sits beside the door. The junction is east. The science lab is west. Medbay is south.',
    properName: true,
  }));
  ...
  scienceLab.add(new IdentityTrait({
    name: 'Science Lab',
    description: 'Workbenches cluttered with broken equipment — a centrifuge tipped on its side, an analyzer with a cracked display. Sealed sample containers line one shelf. A research terminal flickers in the corner, its screen half-corrupted. The corridor is east.',
    properName: true,
  }));
  ...
  medbay.add(new IdentityTrait({
    name: 'Medbay',
    description: 'A medical facility with treatment beds and overhead surgical lights, all dead. A quarantine cell stands in the corner — its door bent outward from inside. Medical supply cabinets line the wall, most ransacked. The lab corridor is north.',
    properName: true,
  }));
  ...
  habCorridor.add(new IdentityTrait({
    name: 'Hab Corridor',
    description: 'The habitation wing. Warmer here, more human. Crew photos line the wall in mismatched frames. A bulletin board holds shift schedules and personal notices — one reads "DO NOT TRUST THE SYSTEM" in hasty marker. The junction is west. The mess hall is east. The library is south.',
    properName: true,
  }));
  ...
  messHall.add(new IdentityTrait({
    name: 'Mess Hall',
    description: 'Long tables bolted to the deck. A food fabrication unit sits against the far wall, dark and silent. Corporate propaganda posters decorate every surface. Coffee mugs with crew names sit abandoned. The hab corridor is west.',
    properName: true,
  }));
  ...
  library.add(new IdentityTrait({
    name: 'Library',
    description: 'The ship\'s data archive. Racks of physical media storage — data crystals, backup drives — fill metal shelving. A comfortable reading chair faces a large terminal display. This feels like the ship\'s memory. The hab corridor is north.',
    properName: true,
  }));
  ...
  upperCorridor.add(new IdentityTrait({
    name: 'Upper Corridor',
    description: 'A narrow corridor with a low ceiling — submarine-tight. Ship schematics are posted behind scratched plexiglass. An intercom panel is mounted by the ladder. Crew bunks are south. The captain\'s cabin is east. The bridge is north, behind a heavy security door.',
    properName: true,
  }));
  ...
  crewBunks.add(new IdentityTrait({
    name: 'Crew Bunks',
    description: 'Stacked sleeping pods, four high. Most are stripped bare. One bunk has deep scratches gouged into the wall beside it. Dark stains on the mattress. A security rack holds equipment near the door. EVA suit storage is recessed into the far wall. The corridor is north. The common area is south.',
    properName: true,
  }));
  ...
  commonArea.add(new IdentityTrait({
    name: 'Common Area',
    description: 'A cramped recreation space. A viewport looks out into black nothing. Someone has built a barricade from furniture and deck plates across the eastern side of the room. A half-finished card game is scattered on a table. An air vent grille is set into the ceiling. The crew bunks are north.',
    properName: true,
  }));
  ...
  captainsCabin.add(new IdentityTrait({
    ...
    description: 'Modest quarters. Personal effects — framed photos, a half-read book, dried flowers in a vase. A locked desk sits against the bulkhead. The corridor is west.',
    properName: true,
  }));
  ...
  bridge.add(new IdentityTrait({
    name: 'Bridge',
    description: 'The nerve center of The Stillwater. A navigation console dominates the forward wall, showing a star chart with a blinking destination marker. A comms array control panel sits beside it. The captain\'s log terminal glows dimly. A security override panel is mounted near the door. The corridor is south.',
    properName: true,
  }));
